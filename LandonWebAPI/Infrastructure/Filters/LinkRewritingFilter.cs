    ...
    private static void RewriteAllLinks(object model, LinkRewriter rewriter)
    {
        ...
        var linkProperties = allProperties
            .Where(property => property.CanWrite &&
                   property.PropertyType == typeof(Link));
        ...
        foreach (var linkProperty in linkProperties)
        {
            ...
            {
                allProperties.SingleOrDefault(property => property.Name == nameof(Resource.Href))?
                    .SetValue(model, rewritten.Href);

                allProperties.SingleOrDefault(property => property.Name == nameof(Resource.Method))?
                    .SetValue(model, rewritten.Method);

                allProperties.SingleOrDefault(property => property.Name == nameof(Resource.Relations))?
